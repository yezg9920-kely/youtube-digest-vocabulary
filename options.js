const YTD_OPTIONS = (() => {
  ...
  const COPY = {
    en: {
      pageTitle: "YouTube Digest Vocabulary Settings",
      ...
      deepseekHelp:
        "YouTube Digest Vocabulary uses DeepSeek V4 Flash for overviews, explanations, translation, and note polishing. ",
      ...
      customizationStepFolder:
        "Open the extracted YouTube Digest Vocabulary project folder in your coding agent.",
      ...
      saved: "Saved. Reopen YouTube Digest Vocabulary to use these settings.",
      saveFailed: "Could not save settings. Please try again.",
      copying: "Copying…",
      promptCopied: "Edited prompt copied.",
      copyFailed:
        "Could not copy the prompt. Select the prompt text and copy it manually.",
      clearedDigests: ({ count, attemptCount }) =>
        `Cleared ${count} cached digest${count === 1 ? "" : "s"} and ${attemptCount} automatic overview attempt record${attemptCount === 1 ? "" : "s"}. Reopen YouTube Digest Vocabulary; the side panel will return to Welcome.`,
      previewClearedDigests: ({ count, attemptCount }) =>
        `Preview only: removed ${count} cached digest${count === 1 ? "" : "s"} and ${attemptCount} automatic overview attempt record${attemptCount === 1 ? "" : "s"} from this page's local preview storage. The installed extension was not changed.`,
      notesDeleted: "Deleted all saved notes.",
      previewNotesDeleted:
        "Preview only: deleted notes from this page's local preview storage. The installed extension was not changed.",
      resetConfirm:
        "Delete API keys, cached digests, translations, saved notes, and vocabulary cards from this Chrome profile?",
      allDataDeleted: "All YouTube Digest Vocabulary data was deleted.",
      previewAllDataDeleted:
        "Preview only: reset this page's local preview storage. The installed extension was not changed.",
      clearCacheFailed:
        "Could not finish clearing cached data. Reopen YouTube Digest Vocabulary before retrying because any open side panel session may have been closed.",
      dataActionFailed: "Could not update local data. Please try again.",
      settingsLoadFailed:
        "Could not load saved settings. You can still preview this page.",
    },
    "zh-CN": {
      pageTitle: "YouTube Digest Vocabulary 设置",
      ...
      deepseekHelp:
        "YouTube Digest Vocabulary 使用 DeepSeek V4 Flash 生成概览、解释内容、翻译字幕和润色笔记。",
      ...
      customizationStepFolder:
        "在编程 Agent 中打开 YouTube Digest Vocabulary 解压后的项目文件夹。",
      customizationStepReplace:
      ...
      customizationPrompt:
        "请把当前本地 YouTube Digest Vocabulary 工作区改为使用 [PROVIDER] 提供的 [MODEL]。只在当前工作区中操作。编辑前，先确认其中包含 manifest.json，且 manifest 中的 name 是 YouTube Digest Vocabulary。如果验证失败，请停止，并让我在编程 Agent 中打开 YouTube Digest Vocabulary 解压后的项目文件夹。不要搜索其他文件夹，不要编辑猜测的副本，不要假设安装路径，也不要声称 Chrome 可以显示操作系统中的绝对源码路径。更新该服务的 API endpoint、请求格式和最少的 Chrome host permissions。保留用户自带密钥模式和 Chrome 本地存储。不要把 API 密钥写入源代码、提交记录、日志、截图、这段提示词或聊天；代码准备好后，请告诉我应该在哪里自行填写密钥。DeepSeek 专用的请求参数和重试逻辑继续只用于 DeepSeek。新服务的专属规则请单独处理，避免相互影响。更新 README.md、README.zh-CN.md、PRIVACY.md、SECURITY.md 和测试。运行 npm test、npm run check 和 npm run package。最后，说明如何重新加载已解压的扩展，并在真实 YouTube 视频上测试。",
      ...
      saved: "已保存。请重新打开 YouTube Digest Vocabulary 以使用这些设置。",
      saveFailed: "无法保存设置，请重试。",
      copying: "正在复制…",
      promptCopied: "已复制编辑后的提示词。",
      copyFailed: "无法复制提示词。请选中提示词文本并手动复制。",
      clearedDigests: ({ count, attemptCount }) =>
        `已清除 ${count} 条缓存摘要和 ${attemptCount} 条自动概览尝试记录。请重新打开 YouTube Digest Vocabulary；侧栏将回到欢迎页。`,
      previewClearedDigests: ({ count, attemptCount }) =>
        `仅预览：已从此页面的本地预览存储中删除 ${count} 条缓存摘要和 ${attemptCount} 条自动概览尝试记录，不会更改已安装的扩展。`,
      notesDeleted: "已删除全部已保存的笔记。",
      previewNotesDeleted:
        "仅预览：已删除此页面本地预览存储中的笔记，不会更改已安装的扩展。",
      resetConfirm:
        "要从当前 Chrome 个人资料中删除 API 密钥、缓存摘要、翻译、已保存的笔记和生词卡（词汇）吗？",
      allDataDeleted: "已删除全部 YouTube Digest Vocabulary 数据。",
      previewAllDataDeleted:
        "仅预览：已重置此页面的本地预览存储，不会更改已安装的扩展。",
      clearCacheFailed:
        "无法完成缓存清理。任何已打开的侧栏会话都可能已关闭；请重新打开 YouTube Digest Vocabulary 后再重试。",
      dataActionFailed: "无法更新本地数据，请重试。",
  ...
  async function requestExtensionDataMutation(root, action) {
    const hasExtensionStorage = Boolean(root?.chrome?.storage?.local);
    if (!hasExtensionStorage) return null;
    const sendMessage = root?.chrome?.runtime?.sendMessage;
    if (typeof sendMessage !== "function") {
      throw new Error("Background mutation failed.");
    }
    let response;
    try {
      response = await sendMessage.call(root.chrome.runtime, { action });
    } catch {
      throw new Error("Background mutation failed.");
    }
  ...
  function dataActionStatusKey(action, result) {
    const keys = {
      clearCache: ["clearedDigests", "previewClearedDigests"],
      deleteNotes: ["notesDeleted", "previewNotesDeleted"],
      resetData: ["allDataDeleted", "previewAllDataDeleted"],
    }[action];
    if (!keys) throw new TypeError("Unknown data action.");
    return result?.previewOnly === true ? keys[1] : keys[0];
  }

  async function clearCachedDigestData(root, storage) {
    const response = await requestExtensionDataMutation(
      root,
      "clearDigestCache",
    );
    if (response) {
      if (
        !isSafeNonnegativeInteger(response.resetEpoch) ||
        !isSafeNonnegativeInteger(response.removedCount) ||
        !isSafeNonnegativeInteger(response.removedAttemptCount)
      ) {
        invalidMutationResponse();
      }
      return {
        success: true,
        resetEpoch: response.resetEpoch,
        removedCount: response.removedCount,
        removedAttemptCount: response.removedAttemptCount,
      };
    }

    const all = await storage.get(null);
    const digestKeys = Object.keys(all)
      .filter((key) => key.startsWith("digest_"))
      .sort();
    const attemptKeys = Object.keys(all)
      .filter((key) => key.startsWith("ytd_overview_attempt_v1_"))
      .sort();
    const keys = [...digestKeys, ...attemptKeys].sort();
    if (keys.length) await storage.remove(keys);
    return {
      success: true,
      removedCount: digestKeys.length,
      removedAttemptCount: attemptKeys.length,
      previewOnly: true,
    };
  }

  async function deleteAllNotesData(root, storage) {
    const response = await requestExtensionDataMutation(root, "deleteAllNotes");
    if (response) {
      if (!isSafeNonnegativeInteger(response.deletedCount)) {
        invalidMutationResponse();
      }
      return { success: true, deletedCount: response.deletedCount };
    }

    const before = await storage.get("ytd_notes");
    const notes = before.ytd_notes;
    await storage.remove("ytd_notes");
    return {
      success: true,
      deletedCount: Array.isArray(notes) ? notes.length : 0,
      previewOnly: true,
    };
  }

  async function resetExtensionData(root, storage, language) {
    const response = await requestExtensionDataMutation(
      root,
      "resetExtensionData",
    );
    if (response) {
      if (!isSafeNonnegativeInteger(response.resetEpoch)) {
        invalidMutationResponse();
      }
      return { success: true, resetEpoch: response.resetEpoch };
    }
  ...
  function initialize(root = globalThis) {
    ...
    async function clearCachedDigests() {
      try {
        const result = await clearCachedDigestData(root, storage);
        setStatus(dataStatus, dataActionStatusKey("clearCache", result), {
          count: result.removedCount,
          attemptCount: result.removedAttemptCount,
        });
      } catch (_error) {
        setStatus(dataStatus, "clearCacheFailed");
      }
    }

    async function clearNotes() {
      try {
        const result = await deleteAllNotesData(root, storage);
        setStatus(dataStatus, dataActionStatusKey("deleteNotes", result));
      } catch (_error) {
        setStatus(dataStatus, "dataActionFailed");
      }
    }
    ...
    async function resetAllData() {
      ...
      try {
        const result = await resetExtensionData(
          root,
          storage,
          currentLanguage,
        );
        await loadSettings();
        setStatus(dataStatus, dataActionStatusKey("resetData", result));
      } catch (_error) {
        setStatus(dataStatus, "dataActionFailed");
      }
    }
