...
- Build understanding with a Chinese deep analysis, argument map, critical review, action ideas, recall questions, chapters, and key quotes.
- Download the transcript as raw TXT or cleaned, timestamp-linked Markdown, then export the analysis alone or as a complete study pack.
- Click an English word to build a bilingual AI memory card with context, a vivid mental scene, and active-recall prompts.
...
Never paste an API key into an AI chat, source file, screenshot, or public message. Enter keys yourself, directly in the YouTube Digest Vocabulary Settings page. Your coding agent can point to the correct field without seeing the key.

## Install manually

If you prefer to do it yourself:

1. Open [github.com/yezg9920-kely/youtube-digest-vocabulary](https://github.com/yezg9920-kely/youtube-digest-vocabulary).
...
8. Pin YouTube Digest Vocabulary from Chrome's Extensions menu if you want quick access.

Because this is an unpacked extension, it does not update automatically. After downloading an update or changing local files, click **Reload** on the YouTube Digest Vocabulary card at `chrome://extensions`, then refresh open YouTube tabs. Moving or deleting the source folder breaks the unpacked extension until you load it again from the new location.

## Set up your API keys

YouTube Digest Vocabulary needs two keys under your own provider accounts:
...
5. Copy the key and paste it into **Supadata API key** in YouTube Digest Vocabulary Settings.
...
3. Choose **Create new API key**, give it a recognizable name such as `YouTube Digest Vocabulary`, and create it.
4. Copy the key immediately. The full key may only be shown once.
5. Paste it into **DeepSeek API key** in YouTube Digest Vocabulary Settings.
...
Open **Settings** from the side panel. You can also open the YouTube Digest Vocabulary **Options** page from its card at `chrome://extensions` or by right-clicking its toolbar icon. Paste keys only into these Settings fields. Never paste a key into an AI chat, repository file, screenshot, or public message.
...
## Use YouTube Digest Vocabulary

1. Open a standard YouTube watch page with captions.
2. Click the YouTube Digest Vocabulary extension icon to open the side panel.
3. Read the timestamped transcript, or choose **Original**, **中文**, or **双语**.
4. Choose **TXT** for a raw TXT transcript, or **Clean MD** for organized paragraphs with timestamp links back to the video.
5. Open **Overview** to generate the Chinese deep analysis. It runs only when you open Overview or choose Regenerate.
6. Choose **Report MD** for the analysis alone, or **Study Pack** for the analysis plus the cleaned transcript in one Markdown file.
7. Select transcript text when you want an AI explanation.
8. Click an English word in the original or bilingual transcript to generate a memory card. Review it, then explicitly choose **Add to vocabulary**.
9. Open **Vocabulary** to search saved words, revisit their video sources, delete cards, or export the current results as CSV, Markdown, and Anki TSV.
10. Save a note from the player or a key quote, then revisit it from **Notes**.

## Download transcripts and deep analysis

The transcript tab offers two different downloads. **TXT** preserves a straightforward raw transcript for archiving or reuse. **Clean MD** groups fragmented captions into readable paragraphs, keeps the first timestamp for each paragraph, and turns that timestamp into a link to the matching point in the YouTube video. This organization is deterministic and does not use an AI model, so you can download it without spending DeepSeek tokens.

The **Overview** tab reads the complete timestamped transcript and produces a Simplified Chinese study report with a one-sentence takeaway, executive summary, core thesis, key insights and their transcript evidence, an argument map, strengths, limitations, hidden assumptions, open questions, action items, active-recall questions, chapters, and source-language quotes. AI can misunderstand a speaker or overstate an inference, so use the timestamp links to check important claims against the transcript and video.

Analysis is lazy: the transcript is sent to DeepSeek only when you open Overview or choose Regenerate. An older cached Overview that contains only chapters and quotes is rebuilt the next time you open the tab. **Report MD** downloads only the normalized analysis. **Study Pack** combines that report with the cleaned transcript for later study, annotation, or import into another notes system. Transcript and report downloads are generated locally and are not uploaded to an export service.

## Learn vocabulary from the transcript

The generated card combines the word's English definition, Chinese meaning, meaning in the original sentence, common collocations, a new example, and a bilingual memory scene. It also creates a cloze question and a production prompt so the card can support active recall instead of rereading alone. Inflected forms such as `running`, `ran`, and `runs` are merged under one lemma when the model resolves them to `run`, while every source sentence and timestamp is retained.

The visual or sound-based mnemonic is a memory cue, not an etymology or pronunciation authority. DeepSeek can make mistakes, so check an authoritative learner dictionary when precision matters. Long-term retention still requires retrieval over time. This first version exports Anki TSV for spaced review but does not schedule reviews inside the extension.

Cards are not saved automatically. A word click sends bounded context to DeepSeek, shows a draft, and stores it locally only after you choose **Add to vocabulary**. Exports are built on your device. Search before exporting if you want a smaller study set.

## What works today

- Google Chrome 116 or newer, using the Side Panel API.
- Standard `youtube.com/watch` video pages.
- Native subtitle tracks returned by Supadata. YouTube Digest Vocabulary prefers English when available, but may show another native language.
- Original, Simplified Chinese, and aligned bilingual transcript views.
- Simplified Chinese deep-analysis reports, cleaned transcript Markdown, complete study-pack export, selected-text explanations, translation, and automatic note polishing.
- Bilingual AI vocabulary memory cards from clickable English transcript words.
- A searchable local vocabulary library with CSV, Markdown, and Anki TSV export.
...
YouTube Digest Vocabulary forces Supadata's `mode=native`. It does not request AI-generated transcripts or perform local audio transcription when native captions are unavailable.
...
- A generated transcript costs **2 credits per video minute**. YouTube Digest Vocabulary does not use this path because it forces `mode=native`.
...
DeepSeek usage is separate from Supadata. DeepSeek may apply its own free quota, rate limits, or charges. YouTube Digest Vocabulary does not collect payments or resell access. Set spending limits and monitor both accounts. The estimate below explains the current DeepSeek translation cost.
...
There is no YouTube Digest Vocabulary account system, advertising, analytics, or telemetry. Supadata and DeepSeek still receive data under their own terms and privacy policies. See [PRIVACY.md](PRIVACY.md) for details.

## Troubleshooting

### The Digest button is missing on a YouTube video

- At `chrome://extensions`, find YouTube Digest Vocabulary and click **Reload**, then refresh the YouTube tab.
...
- At `chrome://extensions`, confirm YouTube Digest Vocabulary is enabled and click **Reload**.
...
### YouTube Digest Vocabulary asks for setup
...
YouTube Digest Vocabulary will not fall back to generated transcription.
