...
YouTube Digest Vocabulary is a GitHub-only, bring-your-own-key Chrome extension. It has no YouTube Digest Vocabulary account, developer-operated backend, analytics, advertising, or telemetry.

## Data the extension handles

Depending on the feature you use, YouTube Digest Vocabulary handles:
...
- a clicked word, its source sentence, bounded nearby context, timestamp, and video metadata when you request a vocabulary memory card;
- transcript context around a timestamped note;
- content you ask to translate;
- notes you save;
- saved vocabulary cards and their source occurrences;
...
YouTube Digest Vocabulary sends the canonical YouTube video URL to `https://api.supadata.ai` with your Supadata API key. Supadata returns the transcript and timestamps. A Supadata key is required for transcript retrieval.

### DeepSeek

The published version sends AI feature content to DeepSeek V4 Flash at `https://api.deepseek.com`:

- the complete timestamped transcript plus relevant video metadata when you open Overview or explicitly regenerate a deep-analysis report;
- selected text plus nearby transcript context for an explanation;
- a clicked word plus its source sentence, bounded nearby transcript context,
  title, and channel when generating a vocabulary memory card;
...
Requests go directly from the extension to Supadata or DeepSeek. They are authenticated with the keys you supply. YouTube Digest Vocabulary's developer does not proxy or receive these requests.
...
YouTube Digest Vocabulary uses Chrome's local extension storage, not a YouTube Digest Vocabulary cloud service.
...
- Vocabulary drafts are not saved automatically. Saved vocabulary cards remain
  locally until you delete them or remove/clear the extension's data.
- Vocabulary exports are generated locally as CSV, Markdown, or Anki TSV and are
  not sent to YouTube Digest Vocabulary or another export service.
- Raw transcript, cleaned transcript, deep-analysis report, and complete study-pack exports are generated locally and are not sent to YouTube Digest Vocabulary or another export service.
...
- delete individual saved notes in YouTube Digest Vocabulary;
- delete individual saved vocabulary cards in YouTube Digest Vocabulary;
...
YouTube Digest Vocabulary uses Chrome permissions for these purposes:

- `sidePanel`: display the YouTube Digest Vocabulary interface beside YouTube.
- `storage`: store settings, keys, notes, vocabulary cards, and cached results locally.
...
- DeepSeek host access: provide AI overviews, explanations, translation,
  vocabulary memory cards, and note polishing through DeepSeek V4 Flash.

YouTube Digest Vocabulary does not use these permissions to monitor general browsing activity.

## No sale or advertising use

YouTube Digest Vocabulary does not sell personal information, build advertising profiles, or share data with data brokers. It does not include analytics SDKs.
