...
function tryInjectNoteButton() {
  ...
  function attempt() {
    ...
    if (attempts >= maxAttempts) {
      debugLog(
        "[YouTube Digest Vocabulary Content] Player container not found after retries, giving up",
      );
      if (ytdNoteButtonRetryTimer) {
        clearInterval(ytdNoteButtonRetryTimer);
        ytdNoteButtonRetryTimer = null;
...
function videoIdentityFailure(message) {
  if (
    ![
      "getVideoInfo",
      "highlightMoments",
    ...
    !Object.hasOwn(message, "expectedVideoId")
  ) {
    return null;
  }
  if (
    typeof message.expectedVideoId !== "string" ||
    !/^[A-Za-z0-9_-]{6,20}$/.test(message.expectedVideoId)
  ) {
    return { success: false, code: "INVALID_EXPECTED_VIDEO_ID" };
  }
  if (currentPageVideoId() !== message.expectedVideoId) {
    return { success: false, code: "VIDEO_ID_MISMATCH" };
  }
  return null;
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  debugLog("[YouTube Digest Vocabulary Content] Received message:", message.action, message);

  const identityFailure = videoIdentityFailure(message);
  if (identityFailure) {
    sendResponse(identityFailure);
    return false;
  }
  ...
  if (message.action === "getVideoInfo") {
    ...
    debugLog("[YouTube Digest Vocabulary Content] Returning video info:", info);
    sendResponse(info);
  ...
  if (message.action === "seekTo") {
    ...
    debugLog("[YouTube Digest Vocabulary Content] Seeking to:", message.seconds);
  ...
  debugLog("[YouTube Digest Vocabulary Content] Unknown action:", message.action);
 ...
 *
 * When clicked, it opens the YouTube Digest Vocabulary side panel.
 */
...
function createDigestButton() {
  ...
  digestButton.setAttribute("aria-label", "Open YouTube Digest Vocabulary");
  ...
  digestButton.addEventListener("click", async (e) => {
    ...
    debugLog("[YouTube Digest Vocabulary] Digest button clicked");
    ...
    try {
      ...
      debugLog("[YouTube Digest Vocabulary] openSidePanel response:", result);
    } catch (err) {
      console.error("[YouTube Digest Vocabulary] Failed to open side panel:", err);
    }
  });
...
function injectDigestButton() {
  ...
  if (!actionsContainer) {
    debugLog("[YouTube Digest Vocabulary Content] Visible actions container not found yet");
    return false;
  }
  ...
  debugLog("[YouTube Digest Vocabulary Content] Digest button reconciled");
  return true;
}
...
function injectNoteButton() {
  ...
  if (!playerContainer) {
    debugLog(
      "[YouTube Digest Vocabulary Content] Player container not found yet, will retry",
    );
    return;
  }
  ...
  debugLog("[YouTube Digest Vocabulary Content] Injecting note button");
  ...
  debugLog("[YouTube Digest Vocabulary Content] Note button injected");
}
...
async function saveCurrentNote() {
  debugLog("[YouTube Digest Vocabulary] Saving note");

  const video = document.querySelector("video.html5-main-video");
  if (!video) {
    console.error("[YouTube Digest Vocabulary] No video element found");
    return;
  }
  ...
  try {
    ...
    if (result.success) {
      ...
      console.error("[YouTube Digest Vocabulary] Save note error:", result.error);
    }
  } catch (err) {
    if (noteButton) {
      noteButton.innerHTML =
    ...
    console.error("[YouTube Digest Vocabulary] Save note exception:", err);
  }
...
function seekToTimestamp(seconds) {
  const video = document.querySelector("video.html5-main-video");
  if (!video) {
    console.error("[YouTube Digest Vocabulary Content] No video element found for seek");
    return;
  }

  debugLog("[YouTube Digest Vocabulary Content] Seeking to:", seconds);
