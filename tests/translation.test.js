...
test("translation cache keys include the complete transcript fingerprint", async () => {
  const helpers = loadSidepanelHelpers();
  const segments = transcriptCore.groupTranscriptEntries([
    { start: 0, duration: 3, text: "A complete source sentence." },
  ]);
  ...
  assert.equal(
    helpers.transcriptTranslationCacheKey(
      segments[0],
      "abc123",
      fingerprint,
    ),
    `abc123:${fingerprint}:zh:semantic:${segments[0].id}`,
  );
});

test("legacy paragraph translations migrate only with same-digest transcript proof", async () => {
  const helpers = loadSidepanelHelpers();
  const segments = transcriptCore.groupTranscriptEntries([
    { start: 0, duration: 3, text: "A complete source sentence." },
  ]);
  ...
  const legacyKey = `abc123:zh:semantic:${segments[0].id}`;
  const newKey = `abc123:${fingerprint}:zh:semantic:${segments[0].id}`;

  const migrated = helpers.prepareCachedParagraphTranslations({
    videoId: "abc123",
    paragraphCache: { [legacyKey]: "已证明属于这份逐字稿。" },
    cachedTranscriptFingerprint: "",
    currentTranscriptFingerprint: fingerprint,
    segments,
  });
  assert.deepEqual(JSON.parse(JSON.stringify(migrated)), {
    entries: [[newKey, "已证明属于这份逐字稿。"]],
    migrated: true,
  });

  const withoutTranscriptProof = helpers.prepareCachedParagraphTranslations({
    videoId: "abc123",
    paragraphCache: { [legacyKey]: "不能复用。" },
    cachedTranscriptFingerprint: "",
    currentTranscriptFingerprint: fingerprint,
    segments: [],
  });
  assert.deepEqual(JSON.parse(JSON.stringify(withoutTranscriptProof)), {
    entries: [],
    migrated: false,
  });
});

test("changed transcript fingerprints isolate current and legacy paragraph caches", async () => {
  const helpers = loadSidepanelHelpers();
  const oldSegments = transcriptCore.groupTranscriptEntries([
    { start: 0, duration: 3, text: "The original complete sentence." },
  ]);
  const changedSegments = transcriptCore.groupTranscriptEntries([
    { start: 0, duration: 3, text: "The changed complete sentence." },
  ]);
  const oldFingerprint = await transcriptCore.fingerprintSegments(oldSegments, {
    sourceLanguage: "en",
    crypto: webcrypto,
  });
  const changedFingerprint = await transcriptCore.fingerprintSegments(
    changedSegments,
    { sourceLanguage: "en", crypto: webcrypto },
  );
  const segmentId = changedSegments[0].id;

  const restored = helpers.prepareCachedParagraphTranslations({
    videoId: "abc123",
    paragraphCache: {
      [`abc123:${oldFingerprint}:zh:semantic:${segmentId}`]: "旧指纹译文",
      [`abc123:zh:semantic:${segmentId}`]: "无证明旧译文",
    },
    cachedTranscriptFingerprint: oldFingerprint,
    currentTranscriptFingerprint: changedFingerprint,
...
test("translated-only omits English while bilingual renders aligned English and Chinese", () => {
  ...
  assert.doesNotMatch(translatedOnly, /data-word="Original"/);
  assert.match(translatedOnly, /\u4e2d\u6587\u8bd1\u6587/);
  assert.match(bilingual, /transcript-original/);
  assert.match(bilingual, /data-word="Original"/);
  assert.match(bilingual, /data-word="sentence"/);
...
test("subtitle formatting tags render in original and translated segment text", () => {
  ...
  assert.match(html, /data-word="Think"/);
  assert.match(html, /<i><button[^>]+data-word="deeply"/);
  assert.match(html, /<b><button[^>]+data-word="carefully"/);
  assert.match(html, /<u><button[^>]+data-word="clearly"/);
  assert.match(html, /<br><button[^>]+data-word="Next"/);
...
test("all AI product requests use DeepSeek non-thinking and JSON behavior", async () => {
  ...
  assert.match(
    backgroundSource,
    /async function handleGenerateBasicOverview\([\s\S]*?prepareBasicOverviewDispatch\([\s\S]*?dispatchPreparedBasicOverview\(/,
  );
});
