...
test("Settings copy covers English and Simplified Chinese", () => {
  assert.equal(options.translate("en", "pageTitle"), "YouTube Digest Vocabulary Settings");
  assert.equal(options.translate("zh-CN", "pageTitle"), "YouTube Digest Vocabulary 设置");
  assert.equal(options.translate("en", "saveSettings"), "Save settings");
  assert.equal(options.translate("zh-CN", "saveSettings"), "保存设置");
  assert.equal(
    options.translate("zh-CN", "clearedDigests", {
      count: 2,
      attemptCount: 1,
    }),
    "已清除 2 条缓存摘要和 1 条自动概览尝试记录。请重新打开 YouTube Digest Vocabulary；侧栏将回到欢迎页。",
  );

  assert.match(
    options.translate("en", "previewClearedDigests", {
      count: 2,
      attemptCount: 1,
    }),
    /Preview only:.*installed extension was not changed/i,
  );
  assert.match(
    options.translate("zh-CN", "previewClearedDigests", {
      count: 2,
      attemptCount: 1,
    }),
    /仅预览：.*不会更改已安装的扩展/,
  );
  assert.match(options.translate("en", "resetConfirm"), /vocabulary cards/i);
  assert.match(options.translate("zh-CN", "resetConfirm"), /生词卡/);
  assert.match(options.translate("en", "clearCacheFailed"), /reopen/i);
  assert.match(options.translate("zh-CN", "clearCacheFailed"), /重新打开/);
...
test("data action status copy distinguishes preview-only storage", () => {
  for (const [action, extensionKey, previewKey] of [
    ...
    assert.equal(options.dataActionStatusKey(action, {}), extensionKey);
    assert.equal(
      options.dataActionStatusKey(action, { previewOnly: true }),
      previewKey,
    );
  }
});
...
test("customization prompt switches languages and preserves technical values", () => {
  ...
  assert.match(chinesePrompt, /^请把当前本地 YouTube Digest Vocabulary 工作区改为使用/);
