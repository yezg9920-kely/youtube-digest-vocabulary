...
function createHarness(options = {}) {
  ...
  const document = {
    ...
    querySelector(selector) {
      if (selector === "video.html5-main-video") return options.video || null;
      return null;
    },
    getElementById(id) {
  ...
  const context = vm.createContext({
    ...
    window: {
      location: {
        pathname: "/watch",
        search: options.locationSearch || "?v=BBBBBB",
      },
      addEventListener(type, listener) {
...
test("content rejects a stale expected video before seeking the player", () => {
  let currentTime = 12;
  let playCalls = 0;
  const video = {
    paused: true,
  ...
  const harness = createHarness({
    locationSearch: "?v=BBBBBB",
    video,
  });
  let response;

  harness.runtimeMessageListeners[0](
    { action: "seekTo", seconds: 90, expectedVideoId: "AAAAAA" },
    {},
    (value) => {
      response = value;
    },
